...
public class NodeGraphics implements GraphicsItem, MouseEventHandler {
    ...
    private void drawTranslucentCircle(Graphics g, int x, int y) {
        ...
        g.fillArc(x, y, NODE_RADIUS * 2, NODE_RADIUS * 2, 0, 360);
    }
    ...
    private Node getClicked(int x, int y) {
        for (Node node : nodes) {
            int centerX = node.screenX + NODE_RADIUS;
            int centerY = node.screenY + NODE_RADIUS;
            if (Math.sqrt(Math.pow(centerX - x, 2) + Math.pow(centerY - y, 2)) < NODE_RADIUS) {
                return node;
            }
        }
        return null;
